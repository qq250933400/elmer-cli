...
const API_HOME_LINK = "https://fanyi.baidu.com";
/**
 * 将中文文本编码为安全的URI字符串。
 * @param {*中文文本词} text 
 */
...
function trans(text) {
    ...
    if (flag) {
        ...
        console.log("[CONNECT][GET]: ", color.green.bold(url_interface));
        // 开始进行网络请求
        ...
        url_interface = "http://fanyi.baidu.com/v2transapi?from=zh&to=en&query=" + text + "&token=b79f2197c91d9b84b6e7eb2e5b9142fc";
        console.log("[CONNECT][GET]: ", color.green.bold(url_interface));
        // 开始处理英文的翻译请求
        superagent.get(url_interface).then((response) => {
            var json = JSON.parse(response.text);
...
function initApi() {
    console.log("[CONNECT][GET]" + color.green.bold(API_HOME_LINK) + '\r\n');
    superagent.get(API_HOME_LINK).then((resp) => {
        console.log("ResponseData", resp.header);
    });
}
/**
