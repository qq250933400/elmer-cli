    ...
    inquirer.prompt([
        {
            ...
            validate: (value) => {
                return isEmpty(value) || /^([a-z0-9]{1,}[a-z0-9\_\-\/\.]*)\@([0-9a-z\_\-\.]*[0-9a-z]{1,}\.([a-z]{1,}))$/i.test(value);
            }
        }
    ]).then((params) => {
...
const saveConfigToFile = (saveFileName, configData) => {
    const sData = "";
    ...
    fs.writeFileSync(targetFileName, sData);
}
const createGitIgnore = (targetFileName, options) => {
    saveConfigToFile(targetFileName, options);
};
const createNpmIgnore = (targetFileName, options) => {
    saveConfigToFile(targetFileName, options);
};
/**
 * 写入文件
